...
from vim_xpath.exceptions import (
    UnknownError,
    XmlBaseError,
    from_lxml_parse_exception,
    from_lxml_xpath_exception,
)
...
def _evaluate(xml, xpath, namespaces=dict(), compiled_xpath=None):
    ...
    for match in tree_matches:
        ...
        if (output_match["line_number"] is None) or output_match[
            "line_number"
        ] <= LIBXML2_MAX_LINE:
    ...
    if line_compress_required:
        ...
        line_compressed_matches = _evaluate(
            line_compressed_xml, xpath, compiled_xpath=compiled_xpath
        )
...
def _output_line_number(match):
    ...
    if isinstance(match, etree.Element):
...
def _output_match(match, namespaces):
    ...
    if isinstance(match, etree.Element):
        ...
        return "<{0}>".format(prefixed_name)

    if isinstance(match, etree._ElementUnicodeResult):
        if match.is_attribute:
            prefixed_name = _prefixed_name_from_absolute_name(
                match.attrname, namespaces
            )
            return "@{0}".format(prefixed_name)
        else:
            return "string"
